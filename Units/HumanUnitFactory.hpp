...
public:
    std::shared_ptr<TownHall> getTownHall(int x, int y) const {
        return std::make_shared<HumanTownHall>(std::dynamic_pointer_cast<const HumanUnitFactory>(shared_from_this()),
                                               x, y);
    }
    
    std::shared_ptr<Barracks> getBarracks(int x, int y) const {
        return std::make_shared<HumanBarracks>(std::dynamic_pointer_cast<const HumanUnitFactory>(shared_from_this()),
                                               x, y);
    }
    
    std::shared_ptr<Worker> getWorker(int x, int y) const {
        return std::make_shared<HumanWorker>(std::dynamic_pointer_cast<const HumanUnitFactory>(shared_from_this()),
                                             x, y);
    }
